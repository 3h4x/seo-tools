...
- `Surface` — standard dashboard panel shell (`rounded-lg`, neutral border/background) for standalone card-like sections. Defaults to `p-5`; use `padding="sm"` for compact `p-4` panels, `padding="xs"` for dense nested `p-3` panels, and `padding="none"` when the child component owns its spacing.
