...
- `Notice` — bordered in-page status banner for operational messages. Supports `warning`, `info`, `neutral`, `danger`, and `success` tones plus `sm`, `md`, card `p-4`, `panel`, `lg`, spacious `p-8`, and unpadded `none` density.
