...
- `Notice` / `NoticeCenteredContent` — bordered in-page status banner for operational messages, plus centered inner content for compact empty/error panels. `Notice` supports `warning`, `info`, `neutral`, `danger`, and `success` tones plus compact `xs`, `sm`, `md`, card `p-4`, `panel`, `lg`, spacious `p-8`, and unpadded `none` density. Use `accent="left"` for left-border status banners instead of hand-rolled `border-l-*` classes.
