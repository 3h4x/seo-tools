export function TrendBadge({ current, previous }: { current: number; previous: number }) {
  ...
  if (previous === 0) {
    return (
      <span className="text-emerald-400 text-[10px] font-medium ml-1" title="New value">
  ...
  return (
    <span className={`text-[10px] font-medium ml-1 ${up ? 'text-emerald-400' : 'text-red-400'}`} title={pctLabel}>
