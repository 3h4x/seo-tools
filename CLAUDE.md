...
2. **Native SQLite shim**: keep `better-sqlite3` / `node:sqlite` loading isolated to `src/lib/sqlite-driver.js`. Do not import those modules elsewhere, do not convert that file to TypeScript, and preserve the `createRequire(path.join(process.cwd(), 'package.json'))` plus lazy `require('node:sqlite')` pattern because Next 16 Turbopack breaks other resolution approaches.
3. **Site config**: use `src/lib/sites.ts` as the boundary for managed site CRUD and normalization. UI and route handlers should call that module instead of reading the `sites` table directly.
4. **Google clients**: keep Search Console logic in `src/lib/search-console.ts`, GA4 logic in `src/lib/ga4.ts`, and service account parsing/auth in `src/lib/google-auth.ts`. Do not expose credentials to client components.
5. **Server/client split**: prefer server components for data loading and small client components only for interaction, sorting, charts, form state, refresh state, and clipboard behavior.
6. **Caching strategy**: all Search Console, GA4, RUM CWV, PSI, and audit fetches should use the SQLite `api_cache` table through `withCache()` unless they are explicit refresh, mutation, or health-check paths. Default TTL is 30 min; PSI uses 6h via the `ttlMs` arg in `cachedGetPagespeed`. The cache PK is `(cache_key, site_id)` — for non-site keys (e.g. PSI by URL), pass the URL as the `site_id` slot. The nav refresh button hits `DELETE /api/cache` which truncates the table; per-namespace clears use `clearCache('psi-')`-style prefix matching.
7. **Background jobs**: long-running recurring work belongs in `src/lib/sitemap-sync.ts` or `src/lib/collect-daily.ts` with CLI wrappers in `scripts/`; keep startup hooks idempotent.
8. **Styling**: use Tailwind v4 classes and the existing dark neutral dashboard style in `app/globals.css` and existing components. Keep operational pages dense and scannable rather than marketing-oriented.
9. **Route handlers**: keep `app/api/**/route.ts` files thin. Parse the request, delegate business logic to `src/lib/**`, and keep response shaping or status-code branching in the handler.
10. **State management**: load dashboard data on the server by default, pass serialized results into client components, and keep client state local to the page or component. Do not introduce a global client state library for fetched data.
11. **Node runtime only**: this app depends on `better-sqlite3`, `node:fs`, `node:path`, and server-only Google clients. Do not move DB/auth/audit code to Edge runtime, client components, or browser bundles.
12. **Page vs lib split**: when page or route code starts accumulating data-massaging logic, move that logic into `src/lib/**` first and test it there. Keep `app/**/page.tsx` focused on orchestration, layout, and rendering.
13. **Shared UI constants**: color palettes and valid parameter sets live in `src/lib/constants.ts` (`CHART_COLORS`, `METRIC_COLORS`, `VALID_DAYS`). Import from there rather than hardcoding hex strings or magic numbers in components or pages.
14. **Domain logic boundaries**: keep audit parsing/scoring in `src/lib/audit.ts`, recommendation generation in `src/lib/gaps.ts`, decay detection in `src/lib/decay.ts`, keyword history math in `src/lib/keyword-history.ts`, GA4 RUM Core Web Vitals aggregation in `src/lib/performance.ts`, per-site CWV data coordination (RUM + PSI fan-out, source detection) in `src/lib/performance-site.ts`, PageSpeed Insights / CrUX field + lab parsing in `src/lib/pagespeed.ts`, display formatting helpers in `src/lib/format.ts`, and discovery/import result types + helpers (bulk-import flow for Config tab) in `src/lib/discovery-import.ts`. CWV thresholds + rating colors live in `src/lib/constants.ts` (`CWV_THRESHOLDS`, `CWV_RATING_COLORS`, `rateCwv()`). Reuse those modules instead of re-implementing the same transformations in pages or components.
15. **Revalidation defaults**: dashboard pages that aggregate cached Google/API data currently use `export const revalidate = 300`. Preserve that five-minute ISR cadence by default and only change it with an explicit freshness or load reason, since manual refresh already exists for forced invalidation.
16. **CLI boundaries**: keep `scripts/*.mjs` focused on argument parsing and orchestration. Put reusable business logic, API calls, and data transforms in `src/lib/**`, then import them into the script instead of duplicating logic across CLI entrypoints.
