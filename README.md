...
Enable **Weekly email digest** in **Config** → Alert Delivery to send a Monday summary after the snapshot job completes. The digest uses the same Resend sender and recipients, compares each site with the same weekday one week earlier, and includes SC clicks, GA4 sessions, audit score, and alert count.
